...
export default function VideoAboutV2() {
  const { sectionRef, glowRef, gridGlowRef, handleMouseMove } = useGridGlow()
  const isMobile = useIsMobile()

  useGSAP(() => {
    gsap.fromTo('.video-about-left',
      { opacity: 0, y: 64 },
      { opacity: 1, y: 0, duration: 1.2, ease: 'expo.out',
        scrollTrigger: { trigger: sectionRef.current, start: 'top 88%' } }
    )
    gsap.fromTo('.video-about-right',
      { opacity: 0, y: 48 },
  ...
  return (
    <section
      ...
      style={{ position: 'relative', background: '#F8F8F4' }}
    >
      ...
      <div style={{
        ...
        {/* Left — project name */}
        <div className="video-about-left" style={{ opacity: 0 }}>
          ...
          <h2 style={{
            ...
            <span style={{ display: 'block' }}>Validación</span>
            <span style={{ display: 'block', color: '#0057FF' }}>RUC-D.</span>
          </h2>
        </div>

        {/* Right — description */}
        <div className="video-about-right" style={{ opacity: 0 }}>
          ...
          <p style={{
            ...
            Las grabaciones pueden impugnarse por tres vías: origen cuestionable, video potencialmente editado y cadena de custodia rota.
          </p>
          ...
          <p style={{
            ...
            El Lab co-diseñó el protocolo RUC-D — un sistema donde falsificar la prueba requiere comprometer simultáneamente tres sistemas independientes: transmisión pública, blockchain y notaría.
          </p>
        </div>
      </div>
    </section>
