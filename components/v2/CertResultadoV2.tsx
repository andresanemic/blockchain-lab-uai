...
const STATS = [
  { label: 'Participantes del piloto', value: '30' },
  { label: 'Por certificado',           value: '< 10s' },
  { label: 'Costo en mainnet',          value: '< $0.01' },
]

const H2_LINES = [
  { text: 'El piloto lo demostró:', accent: false },
  { text: 'la confianza verificable', accent: false },
  { text: 'es posible.',             accent: true  },
]

export default function CertResultadoV2() {
  const { sectionRef, glowRef, gridGlowRef, handleMouseMove } = useGridGlow(true)
  const isMobile = useIsMobile()

  useGSAP(() => {
    gsap.fromTo('.cert-res-eyebrow',
      { opacity: 0, y: 16 },
      { opacity: 1, y: 0, duration: 0.8, ease: 'expo.out',
        scrollTrigger: { trigger: sectionRef.current, start: 'top 92%' } }
    )
    gsap.fromTo('.cert-res-line',
      { y: '108%', opacity: 0, filter: 'blur(12px)' },
      {
        y: '0%', opacity: 1, filter: 'blur(0px)',
        duration: 1.2, ease: 'expo.out', stagger: 0.10, delay: 0.1,
        scrollTrigger: { trigger: sectionRef.current, start: 'top 90%' },
      }
    )
    gsap.fromTo('.cert-res-stat',
      { opacity: 0, y: 40, scale: 0.97 },
      { opacity: 1, y: 0, scale: 1, duration: 1.0, ease: 'expo.out',
        stagger: { amount: 0.4, from: 'start' }, delay: 0.25,
        scrollTrigger: { trigger: '.cert-res-stat', start: 'top 90%' } }
    )
