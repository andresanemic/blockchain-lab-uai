...
const stats = [
  { value: 'Web2 + Web3', label: 'Capas tecnológicas' },
  { value: '4 capas', label: 'Verificación independiente' },
  { value: 'SHA-256', label: 'Integridad del video' },
]

export default function ValidHero() {
  ...
  useGSAP(
    () => {
      ...
      tl.from('.valid-hero-text > *', { y: 40, opacity: 0, duration: 0.9, stagger: 0.12 })
      tl.from(orbRef.current, { opacity: 0, scale: 0.6, duration: 1.2, ease: 'power2.out' }, 0.2)
      tl.from('.valid-stat', { y: 20, opacity: 0, duration: 0.6, stagger: 0.1 }, 0.5)
    },
    { scope: containerRef }
  )
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 w-full py-20">
        <div className="valid-hero-text max-w-3xl">
          ...
          <h1
            ...
            style={{ color: '#F0F0F5' }}
          >
            Pruebas multimedia que no se pueden{' '}
            <span style={{ color: '#3B5BDB' }}>impugnar</span>
          </h1>

          <p className="text-lg leading-relaxed max-w-2xl mb-10" style={{ color: '#9898B0' }}>
            Un sistema que combina transmisión en vivo, registro blockchain y gobernanza
            institucional para transformar grabaciones de dron en evidencia judicial
            técnica y procesalmente irrefutable.
          </p>
        ...
        <div className="mt-16 grid grid-cols-3 gap-px" style={{ background: '#1E1E2E' }}>
          {stats.map((s) => (
            <div key={s.value} className="valid-stat flex flex-col gap-1 px-6 py-5" style={{ background: '#0A0A0F' }}>
              <span className="font-mono font-bold text-xl lg:text-2xl" style={{ color: '#3B5BDB' }}>
                {s.value}
              </span>
